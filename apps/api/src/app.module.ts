...
@Module({imports:[ConfigModule.forRoot({isGlobal:true,validate:validateEnvironment,envFilePath:[resolve(__dirname,'../.env'),resolve(__dirname,'../../../.env')]}),ThrottlerModule.forRoot([{ttl:60000,limit:60}]),LoggerModule.forRoot({pinoHttp:{level:process.env.LOG_LEVEL||'info',redact:{paths:['req.headers.authorization','req.headers.cookie','res.headers.set-cookie','req.body.password','req.body.currentPassword','req.body.newPassword','req.body.token'],censor:'[REDACTED]'},genReqId:req=>req.headers['x-request-id']?.toString()||crypto.randomUUID()}}),DatabaseModule,AuthModule,HealthModule,RolesModule,AcademicsModule,FeesModule,PaymentsModule,ExpenditureModule,ReportingModule],controllers:[AppController],providers:[{provide:APP_GUARD,useClass:ThrottlerGuard},{provide:APP_GUARD,useClass:AuthenticationGuard},{provide:APP_GUARD,useClass:PermissionsGuard}]})
